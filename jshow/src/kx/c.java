...
static{formatter.setTimeZone(java.util.TimeZone.getTimeZone("GMT"));}
static synchronized String sd(String s, java.util.Date x){formatter.applyPattern(s);return formatter.format(x);}
public static String sd(Date x){return sd("yyyy.MM.dd", x);}
public static String sd(Time x){return sd("HH:mm:ss.SSS", x);}
public static String sd(Timestamp x){return sd("yyyy.MM.dd HH:mm:ss.SSS", x);}
...
public c(Socket s,IAuthenticate a)throws IOException{io(s);int n=i.read(b=new byte[99]);if(a!=null&&!a.authenticate(new String(b,0,n>1?n-2:0))){close();throw new IOException("access");}vt=n>1?b[n-2]:0;b[0]=(byte)(vt<'\3'?vt:'\3');o.write(b,0,1);} //c c=new c(new ServerSocket(5010));while(true)c.w(2,c.k());
public c(ServerSocket s,IAuthenticate a)throws IOException{this(s.accept(),a);}
