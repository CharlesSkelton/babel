/************************************************
 * This file, c.cs, is copyright Kx Systems and updates
 * can be found at http://www.kx.com/k4/c
*************************************************/
// 2004.10.12 usr:pwd
// 2004.09.07 simultaneous read(k()) and write(ks())
// 2004.09.06 bigendian reads
using System;using System.IO; //csc c.cs valuetypes(DateTime(dz)TimeSpan(t))
class c:System.Net.Sockets.TcpClient{public static void Main(string[]args){
 //c c=new c("localhost",5010);c.k(".u.sub[`trade;`INTC`CSCO]");while(true){object r=c.k();O(n(at(r,2)));}
 c c=new c("localhost",5001,"usr:pwd");
 object[]x=new object[4];x[0]=t();x[1]="xx";x[2]=(double)93.5;x[3]=300;
 Flip r=td(c.k("select sum price by sym from trade"));O(n(r.x));O(n(r.y[0]));     //cols rows
// tm();for(int i=0;i<1000;++i)c.ks("insert", "trade", x);tm();
}
byte[]b,B;int j,J;bool a;Stream s;public c(string h,int p):this(h,p,".net"){}
public c(string h,int p,string u):base(h,p){s=this.GetStream();B=new byte[1+u.Length];J=0;w(u);
 s.Write(B,0,J);if(1!=s.Read(B,0,1))throw new Exception("access");}
...
static int dt(DateTime d){return d==d.Date?14:15;}static int DT(DateTime[]d){int i=0;for(;i<n(d);++i)if(15==dt(d[i]))return 15;return 14;}
static DateTime dz(double z){long l=(long)(.5+8.64e11*z)+o;return new DateTime(l<0?0L:l);}
static double zd(DateTime d){long l=d.Ticks;return l==0?Double.NaN:(l-o)/8.64e11;}static long o=(long)8.64e11*730119;
...
static int t(object x){return x is bool?-1:x is byte?-4:x is short?-5:x is int?-6:x is long?-7:x is float?-8:x is double?-9:x is char?-10:
 x is string?-11:x is Month?-13:x is DateTime?-dt((DateTime)x):x is Minute?-17:x is Second?-18:x is TimeSpan?-19:
...
static int[]nt={0,1,0,0,1,2,4,8,4,8,1,0,0,4,4,8,4,4,4,4};
static int n(object x){int t=c.t(x);return t==99?n(((Dict)x).x):t==98?n(((Flip)x).y[0]):((Array)x).Length;}
...
void w(DateTime d){w((int)zd(d));}DateTime rd(){return dz((double)ri());}void W(DateTime z){w(zd(z));}DateTime rz(){return dz(rf());}
